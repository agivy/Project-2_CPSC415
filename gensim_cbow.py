...
def save_top_disparities(disparate_pairs, filename='top_100_disparities_gensim_cbow.csv', n=1000):
...
def plot_disparities(disparate_pairs, n=100):
    ...
    # Save plot
    plt.tight_layout()
    plt.savefig('word_pair_disparities.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    model1.save("cbow_model1.model")
    model2.save("cbow_model2.model")
    print("\nModels saved in gensim format")
    
    # Save parameters, vectors, and loss history
    save_model_params(model1, loss_history1, "cbow_model1_params.json")
    save_model_params(model2, loss_history2, "cbow_model2_params.json")
